...
import { useState, useEffect, useRef } from "react";
import { X, Send, Mic, MessageCircle, User, Bot, Camera, Upload } from "lucide-react";
...
const ChatPanel = ({ isOpen, onClose }: ChatPanelProps) => {
  ...
  const isPlayingRef = useRef(false);
  const nextPlayTimeRef = useRef(0);
  const [currentSampleRate, setCurrentSampleRate] = useState(8000);
  ...
  useEffect(() => {
    ...
    websocket.onopen = () => {
      ...
      websocket.send(JSON.stringify({ type: "create_session" }));
    };
  ...
  // Send attach_session control message first
  if (sessionId) {
    socket.send(JSON.stringify({
      type: "attach_session",
      session_id: sessionId
    }));
    console.log("📤 Sent attach_session with ID:", sessionId);
  } else {
    console.error("⚠️ No session ID available for audio attachment");
  }
  ...
  const handleWebSocketMessage = (data: WebSocketMessage) => {
    ...
    case "subscribed":
      ...
      if (newSessionId) {
        ...
        sessionIdRef.current = newSessionId;  // ADD THIS LINE
  ...
  const sendImageToBackend = async (base64Image: string, tempMessageId: string) => {
   ...
   const messageData: any = {
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (response.success && response.image) {
            ...
            setMessages(prev => prev.map(msg => {
              if (msg.id === tempMessageId) {
                return { ...msg, text: "❌ Failed to upload image" };
              }
              return msg;
            }));
          ...
          setMessages(prev => prev.map(msg => {
            if (msg.id === tempMessageId) {
              return { ...msg, text: "❌ Error processing response" };
            }
            return msg;
          }));
          appendMessage("assistant", "Error processing server response.");
      ...
      socket.onerror = (err) => {
        ...
        setMessages(prev => prev.map(msg => {
          if (msg.id === tempMessageId) {
            return { ...msg, text: "❌ Failed to send image" };
          }
          return msg;
        }));
        appendMessage("assistant", "Failed to send image. Please try again.");
      ...
      setMessages(prev => prev.map(msg => {
        if (msg.id === tempMessageId) {
          return { ...msg, text: "❌ Error processing image" };
        }
        return msg;
      }));
      appendMessage("assistant", "Error processing image. Please try again.");
